(() => {
  ...
  const UI_BINDINGS = [
    ...
    ['#makeButton span','make'],['#makeButton small','make.note'],['#advancedButton','advanced.open'],['#projectIoTitle','io.heading'],['#exportSceneButton strong','io.export'],['.project-io-import strong','io.import'],['.footer-note','footer.note'],
  ...
  function syncEasyShellToWorkingDocument(){
    ...
    workingDocument.title=titleInput.value.trim() || workingDocument.title || 'Untitled';
  ...
  function renderSceneList(){
    ...
    workingDocument.scenes.forEach((scene,i)=>{
      ...
      b.innerHTML=`<span>${String(i+1).padStart(2,'0')}</span><div><strong>${scenePreviewText(scene)}</strong><small>${typeLabel} · ${effectLabel}${sceneLang?' · '+sceneLang.toUpperCase():''}${media.length?' · '+media.join('/') : ''}</small></div>`;
  ...
  $('#sampleButton').addEventListener('click',()=>{titleInput.value='声のそろう通り';bodyInput.value=SAMPLE;easySourceDirty=true;updateCount();});
  ...
  cinemaInput.addEventListener('change',()=>{const file=cinemaInput.files?.[0];if(!file)return;if(cinemaBackgroundUrl && !String(cinemaBackgroundUrl).startsWith('blob:'))URL.revokeObjectURL(cinemaBackgroundUrl);cinemaBackgroundUrl=URL.createObjectURL(file);cinemaPreview.style.backgroundImage=`url("${cinemaBackgroundUrl}")`;cinemaPreview.hidden=false;cinemaClear.hidden=false;if(workingDocument?.scenes?.[0]){const p=ensurePresentation(workingDocument.scenes[0]);p.background={src:cinemaBackgroundUrl,transition:'fade',dim:cinemaTone==='dark'?0.48:0.72,fit:'cover',position:'center center',_editorFileName:file.name,_editorManaged:true};}});
  cinemaClear.addEventListener('click',()=>{cinemaBackgroundUrl='';cinemaInput.value='';cinemaPreview.style.backgroundImage='';cinemaPreview.hidden=true;cinemaClear.hidden=true;if(workingDocument?.scenes?.[0]){const p=ensurePresentation(workingDocument.scenes[0]);delete p.background;}});
  ...
  window.SceneStudioDebug={getSceneDocument:()=>clone(workingDocument||buildSceneDocument()),validateSceneFormatV1:(value)=>validateSceneFormatV1(value),exportSceneDocument,getPlayer:()=>player,splitJapanese:(text,options={})=>JapaneseSceneSplitter.splitDetailed(text,options),splitEnglish:(text,options={})=>EnglishSceneSplitter.splitDetailed(text,options),splitAuto:(text,options={})=>SceneTextSplitter.splitDetailed(text,options),splitMultilingual:(text,options={})=>SceneTextSplitter.splitMultilingualDetailed(text,options),summarizeLanguages:(chunks)=>SceneTextSplitter.summarizeLanguages(chunks),detectWorkLanguage:(text)=>SceneTextSplitter.detectLanguage(text),getUILanguage:()=>uiLanguage,setUILanguage};
