(function(global){
  ...
  const M = {
    ja: {
      ...
      'advanced.open':'シーンを調整','io.heading':'作品ファイル','io.export':'作品を書き出す','io.import':'作品を読み込む','io.exported':'書き出しました：{name}','io.imported':'読み込みました：{name}（{n} Scenes）','io.invalid':'Scene Format v1として読み込めません。','io.localAssets':'ローカル画像・音声の参照が{n}件あります。同じブラウザセッション外では再選択が必要です。','footer.note':'Easy Studioは本文を作るだけ。再生は共通Scene Player Coreが担当します。',
      ...
      'background.choose':'画像を選ぶ','background.unselect':'選択解除','background.transition':'切替','background.fit':'表示','background.motion':'動き','background.dim':'暗さ','background.thin':'薄さ',
    ...
    en: {
      ...
      'advanced.open':'Adjust Scenes','io.heading':'Work file','io.export':'Export work','io.import':'Import work','io.exported':'Exported: {name}','io.imported':'Imported: {name} ({n} Scenes)','io.invalid':'Could not import this as Scene Format v1.','io.localAssets':'This work contains {n} local image/audio reference(s). They must be selected again outside the current browser session.','footer.note':'Easy Studio only authors the content. Shared Scene Player Core handles playback.',
      ...
      'background.choose':'Choose image','background.unselect':'Clear selection','background.transition':'Transition','background.fit':'Fit','background.motion':'Motion','background.dim':'Darkness','background.thin':'Fading',
