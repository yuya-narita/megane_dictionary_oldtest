/* 122_peace_shooter_v02.js
   SYNTAX FRONTIER INC.
   BUG No.012 — PEACE SHOOTER v0.2
...
(function(){
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .ps-controls{
        display:grid;grid-template-columns:1fr 1fr 1.2fr;gap:10px;width:min(100%,560px);margin:0 auto;
      }
      .ps-control{
        min-height:56px;border:2px solid rgba(202,231,173,.45);
        background:rgba(202,231,173,.06);color:#cae7ad;font:inherit;font-weight:1000;font-size:17px;letter-spacing:.08em;
      }
      .ps-control.fire{border-color:rgba(255,190,170,.65);color:#ffd0bf}
      .ps-control:active{background:rgba(202,231,173,.18)}
  ...
  function open(){
    ...
    root.querySelector(".ps-control.fire").addEventListener("pointerdown",function(e){e.preventDefault();e.stopPropagation();fire()},true);
  ...
  function loop(now){
    ...
    // First hit destroys one enemy. Only then does WAR begin.
    state.playerBullets.forEach(function(b){
      state.enemies.forEach(function(en){
        if(!en.alive)return;
    ...
    if(state.playerX>=88&&state.playerY<=30){
      clearGame();
      return;
    }
  ...
  window.MEGANE_PEACE_SHOOTER={version:"v0.2",open:open,close:close};
})();