/* 137_recursion_bug_v15.js
   SYNTAX FRONTIER INC.
   BUG No.023 — RECURSION BUG
   ENDLESS STACK v1.5 / DEV LAYOUT + SCROLL FIX
...
(function(){
  ...
  function ensureStyle(){
    if(document.getElementById("recursionBugStyleV15"))return;
    var st=document.createElement("style");st.id="recursionBugStyleV15";
    st.textContent=`
  ...
  function installZoomGuard(){
    ...
    root.addEventListener("touchend",stopRootTouch,{capture:true,passive:false});
  }
  ...
  function removeZoomGuard(){
    if(zoomGuardHandlers){
      ...
      if(root&&zoomGuardHandlers.stopRootTouch){
        ...
        root.removeEventListener("touchend",zoomGuardHandlers.stopRootTouch,true);
      }
  ...
  function open(){
    ...
    $(".rb-resume").addEventListener("pointerup",function(e){block(e);resume()},true);$(".rb-dev-menu").addEventListener("pointerup",function(e){block(e);openDeveloper()},true);$(".rb-restart").addEventListener("pointerup",function(e){block(e);start()},true);$(".rb-quit").addEventListener("pointerup",stopEventClose,true);$(".rb-ending-return").addEventListener("pointerup",stopEventClose,true);
  ...
  function bindDeveloperPanel(){
    var header=root.querySelector(".rb-head");
    ...
    function beginPress(e){
      if(e&&e.target&&e.target.closest(".rb-head-actions"))return;
    ...
    header.addEventListener("pointerdown",beginPress,true);
    header.addEventListener("pointerup",finishPress,true);
    header.addEventListener("pointercancel",finishPress,true);
    header.addEventListener("lostpointercapture",finishPress,true);

    header.addEventListener("touchstart",beginPress,{capture:true,passive:false});
    header.addEventListener("touchend",finishPress,{capture:true,passive:false});
    header.addEventListener("touchcancel",finishPress,{capture:true,passive:false});

    // Fallback: tap the title text five times quickly.
    var tapCount=0,tapReset=0;
    titleText.addEventListener("click",function(e){
      ...
      tapReset=setTimeout(function(){tapCount=0},1200);
    },true);
  ...
  window.MEGANE_RECURSION_BUG={version:"v1.5-dev-layout-scroll-fix",open:open,close:close};
})();
