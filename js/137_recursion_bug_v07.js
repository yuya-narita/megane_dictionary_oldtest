/* 137_recursion_bug_v07.js
   SYNTAX FRONTIER INC.
   BUG No.023 — RECURSION BUG
   ENDLESS STACK v0.7 / PHASE 3
...
(function(){
  ...
  function ensureStyle(){
    if(document.getElementById("recursionBugStyleV07"))return;
    var st=document.createElement("style");st.id="recursionBugStyleV07";
    st.textContent=`
  ...
  function open(){
    ...
    root.innerHTML=`<div class="rb-shell">
      ...
      <main class="rb-main"><div class="rb-stage"><canvas class="rb-canvas"></canvas><div class="rb-scan"></div>
        ...
        <div class="rb-next"><div class="rb-next-label">NEXT</div><canvas class="rb-next-canvas"></canvas></div><div class="rb-world-label">FIELD 10×20</div><div class="rb-event"><div class="rb-event-panel"><div class="rb-event-title"></div><div class="rb-event-copy"></div></div></div>
  ...
  function spawn(forcedType){var type=forcedType||state.nextType||nextFromBag();if(!forcedType)state.nextType=nextFromBag();state.piece={type:type,cells:cloneCells(SHAPES[type]),x:0,y:-1};normalize(state.piece);state.piece.x=Math.floor((COLS-pieceWidth(state.piece))/2)-minX(state.piece);state.grounded=false;state.lockClock=0;state.lockResets=0;state.hardDropped=false;state.swapUsed=false;if(collides(state.piece,0,0)){if(state.verticalExpansions<2){state.piece=null;expandVertical(type);return}else gameOver()}drawNext()}
