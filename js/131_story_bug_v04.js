/* 131_story_bug_v04.js
   SYNTAX FRONTIER INC.
   No.017 — STORY BUG v0.4
...
(function(){
  ...
  function startBgm(){
    ...
    function tick(){
      var ctx=ensureAudio();
      if(!ctx||!battleBgmActive||!root)return;
      var now=ctx.currentTime+.02;
      var f=seq[bgmStep%seq.length];
      note(f,.38,.023,"sawtooth",now);
      note(f*2,.19,.008,"square",now+.04);
      if(bgmStep%4===0)note(164.8,.5,.005,"triangle",now+.08);
      bgmStep++;
    }
    tick();
      ...
      <div class="sb-shell">
        ...
        <div class="sb-loading">
          ...
          <div class="sb-loading-panel">
            <div class="sb-rom">ROM 017 / STORY BUG / v0.4</div>
