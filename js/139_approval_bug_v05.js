...
(function(){
  ...
  var VERSION="v0.5-profile-tabs-comments-delete";
  ...
  var root=null,state=null,audio=null,raf=0,timers=[],dbPromise=null,saveTimer=0,saveChain=Promise.resolve(),booting=false,mediaGesture=null,profileTab="posts",pressTimer=0,pressedPostId=null;
  ...
  function ensureStyle(){
    if(document.getElementById("approvalBugStyleV05"))return;
    var s=document.createElement("style");
    s.id="approvalBugStyleV05";
    s.textContent=`
      ...
      #approvalBugRoot .ab-media-grid button{aspect-ratio:1/1;border:0;padding:0;background:#111;overflow:hidden}
      #approvalBugRoot .ab-media-grid img{width:100%;height:100%;object-fit:cover;display:block}
  ...
  function renderProfile(){
    ...
    if(profileTab==="images"){
      body=mediaPosts.length
        ? '<div class="ab-media-grid">'+mediaPosts.slice().reverse().map(function(post){return '<button type="button" class="ab-profile-media" data-src="'+escapeHtml(post.image)+'"><img src="'+post.image+'" alt=""></button>'}).join("")+'</div>'
  ...
  function createPost(text,imageData){
    ...
    var post={
      ...
      image:imageData||"",selfLiked:false
    };
    state._pendingPostImage="";
