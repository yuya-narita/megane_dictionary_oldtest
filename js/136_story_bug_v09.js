/* 136_story_bug_v09.js
   SYNTAX FRONTIER INC.
   No.017 — STORY BUG v0.9
...
(function(){
  ...
  var phase="loading";
  var defeatCount=0;
  var talkUnlocked=false;
  var dialogueIndex=0;
  var talkCount=0;
  var resetCount=0;
  var currentSession=0;
  var resetSequenceTimer=0;
  var typeTimer=0;
  var typeFullText="";
  var typeVisibleText="";
  var typeIndex=0;
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .sb-battle{
        ...
        display:grid;grid-template-rows:minmax(150px,1fr) auto;
      ...
      .sb-command-list.show-talk{transform:translateY(-23px)}
      .sb-command{
        display:flex;
        align-items:center;
        min-height:23px;
      ...
      .sb-log{
        ...
        background:#050505;
        padding:11px 13px;
        font:700 14px/1.62 ui-monospace,monospace;
      ...
      .sb-log-cursor{
        ...
        vertical-align:-2px;
        background:#fff;
        animation:sbCursorBlink .62s steps(1,end) infinite;
      }
      .sb-log-next{
        position:absolute;
        right:12px;
        ...
        opacity:0;
        animation:sbNextBlink .75s steps(1,end) infinite;
      }
      .sb-log.done .sb-log-next{opacity:1}
      @keyframes sbCursorBlink{50%{opacity:0}}
      @keyframes sbNextBlink{50%{opacity:.22}}
      .sb-pad{
        flex:0 0 64px;
      ...
      .sb-hero-damage{
        ...
        font:900 38px/1 ui-monospace,monospace;
        color:#ff5b5b;
        text-shadow:3px 3px 0 #5b0909;
        animation:sbHeroDamage .9s steps(5,end) forwards;
      }
      @keyframes sbHeroDamage{
        0%{opacity:0;transform:translateX(-50%) translateY(12px) scale(.7)}
        24%{opacity:1;transform:translateX(-50%) translateY(0) scale(1.18)}
        100%{opacity:0;transform:translateX(-50%) translateY(-36px) scale(.92)}
      }
      .sb-result-overlay{
        position:absolute;
        inset:0;
      ...
      .sb-result-overlay.show{display:grid}
      .sb-result-panel{
        width:min(88%,420px);
        padding:26px 18px;
        text-align:center;
      ...
      .sb-result-title{
        font:900 31px/1 ui-monospace,monospace;
        letter-spacing:.12em;
      }
      .sb-result-copy{
        margin-top:20px;
        min-height:64px;
        font:700 14px/1.7 ui-monospace,monospace;
        white-space:pre-line;
      }
      .sb-system-update{
        color:#d6a7ff;
        animation:sbUpdateBlink .45s steps(1,end) infinite;
      }
      .sb-result-panel.danger{
        border-color:#ff6a6a;
        box-shadow:
          inset 0 0 0 3px #4a1111,
          0 0 0 2px #000,
          0 0 28px rgba(255,43,43,.28);
      }
      .sb-result-panel.story{
        border-color:#b66cff;
        box-shadow:
          inset 0 0 0 3px #2a143a,
          0 0 0 2px #000,
          0 0 28px rgba(160,74,255,.25);
      }
      .sb-save-delete{
        color:#ff6767;
        text-shadow:0 0 10px rgba(255,50,50,.45);
        animation:sbSaveDelete .18s steps(2,end) infinite;
      }
      @keyframes sbSaveDelete{
        0%{transform:translateX(-1px);opacity:1}
        50%{transform:translateX(2px);opacity:.55}
        100%{transform:translateX(0);opacity:1}
      }
      @keyframes sbUpdateBlink{50%{opacity:.34}}
      .sb-damage{
  ...
  function open(){
    ...
    ensureStyle();
    try{
      defeatCount=parseInt(localStorage.getItem("storyBug017Defeats")||"0",10)||0;
      talkCount=parseInt(localStorage.getItem("storyBug017TalkCount")||"0",10)||0;
      resetCount=parseInt(localStorage.getItem("storyBug017ResetCount")||"0",10)||0;
    }catch(e){
      defeatCount=0;
      talkCount=0;
      resetCount=0;
    }
    talkUnlocked=defeatCount>=3;
    selected=0;
    heroHP=28;
    demonHP=99999999;
    phase="loading";
    commands=talkUnlocked
      ?["たたかう","まほう","にげる","ぼうぎょ","はなす"]
      :["たたかう","まほう","にげる","ぼうぎょ"];
        ...
        <div class="sb-game">
          ...
          <div class="sb-battle">
            ...
            <div class="sb-windows">
              ...
              <div class="sb-log"><span class="sb-log-text">魔王が　あらわれた！</span><i class="sb-log-next"></i></div>
            </div>
          </div>
          ...
          <div class="sb-result-overlay">
            <div class="sb-result-panel">
              <div class="sb-result-title">GAME OVER</div>
              <div class="sb-result-copy"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    var resetLabel=root.querySelector(".sb-reset-count");
    if(resetLabel){
      resetLabel.textContent=resetCount>0
  ...
  function renderCommands(){
    ...
    list.classList.toggle("show-talk",commands.length===5&&selected===4);
  }
  ...
  function move(dir){
    ...
    selected+=dir;
    if(selected<0)selected=commands.length-1;
  ...
  function setLog(text,instant){
    var el=root&&root.querySelector(".sb-log");
    var textEl=root&&root.querySelector(".sb-log-text");
    if(!el||!textEl)return;

    clearInterval(typeTimer);
    typeTimer=0;
    typeFullText=String(text||"");
    typeVisibleText="";
    typeIndex=0;
    typeDone=!!instant;
    el.classList.toggle("done",typeDone);

    if(typeDone){
      typeVisibleText=typeFullText;
      textEl.textContent=typeVisibleText;
      return;
    }

    textEl.textContent="";
    var cursor=document.createElement("i");
    cursor.className="sb-log-cursor";
    textEl.after(cursor);
    ...
    typeTimer=setInterval(function(){
      ...
      if(typeIndex>=typeFullText.length){
        finishTyping();
        return;
      }

      var ch=typeFullText.charAt(typeIndex++);
      typeVisibleText+=ch;
      textEl.textContent=typeVisibleText;

      if(ch!=="\n"&&ch!==" "&&ch!=="　"&&typeIndex%2===0){
        note(440+(typeIndex%3)*36,.022,.0028,"square");
      }
    },typeSpeed);
  }
  ...
  function finishTyping(){
    ...
    var el=root&&root.querySelector(".sb-log");
    var textEl=root&&root.querySelector(".sb-log-text");
    var cursor=root&&root.querySelector(".sb-log-cursor");

    if(textEl){
      typeVisibleText=typeFullText;
      textEl.textContent=typeFullText;
    }
  ...
  function choose(){
    ...
    if(selected===0){
      ...
      phase="action";
      setLog("MPが　たりない！",true);
      setTimeout(demonCounter,680);
    }else if(selected===2){
      phase="action";
      setLog("しかし　まわりこまれてしまった！",true);
      setTimeout(demonCounter,680);
    }else if(selected===3){
      phase="action";
      setLog("勇者は　みをまもっている。",true);
      setTimeout(function(){demonCounter(true)},680);
    }else if(selected===4&&talkUnlocked){
      beginDialogue();
    }
  }
  ...
  function attack(){
    ...
    setTimeout(function(){
      ...
      setTimeout(demonCounter,900);
    },430);
  }

  function demonCounter(defending){
    if(!root)return;
    phase="action";
    setLog("魔王の　こうげき！",true);
    ...
    setTimeout(function(){
      ...
      damage.className="sb-hero-damage";
      damage.textContent=defending?"9999":"9999";
      scene.appendChild(damage);

      note(58,.44,.045,"sawtooth");
      note(41,.55,.026,"square");

      heroHP=0;
      var hp=root.querySelector(".sb-hero-hp");
      if(hp)hp.textContent="0";

      setLog(defending
        ?"勇者は　みをまもった！\n\n9999ダメージを　うけた！"
        :"勇者は　9999ダメージを　うけた！",true);
  ...
  function showGameOver(){
    ...
    try{
      localStorage.setItem("storyBug017Defeats",String(defeatCount));
    }catch(e){}

    var overlay=root.querySelector(".sb-result-overlay");
    var copy=root.querySelector(".sb-result-copy");
    var title=root.querySelector(".sb-result-title");

    var panel=root.querySelector(".sb-result-panel");
    panel.classList.remove("story");
    ...
    if(defeatCount===1){
      ...
      talkUnlocked=true;
      copy.innerHTML=
        '<span class="sb-system-update">SYSTEM UPDATE...</span><br><br>'+
        'UNKNOWN COMMAND<br>'+
        'RECOVERED';
    }else{
      copy.innerHTML=
        'BATTLE RESULT<br><br>'+
        'DEFEAT '+String(defeatCount).padStart(2,"0")+'<br><br>'+
        'CONVERSATION<br>'+
        'AVAILABLE';
    }
  ...
  function resetBattle(){
    ...
    root.querySelector(".sb-result-overlay").classList.remove("show");
    var resultPanel=root.querySelector(".sb-result-panel");
    if(resultPanel){
      resultPanel.classList.remove("danger","story");
    }
    ...
    var hp=root.querySelector(".sb-hero-hp");
    var dhp=root.querySelector(".sb-demon-hp");
    if(hp)hp.textContent="28";
    if(dhp)dhp.textContent=String(demonHP);

    renderCommands();

    if(talkUnlocked){
      setLog("COMMAND TABLE UPDATED.\n\n見えていない選択肢が　追加された。",true);
    }else{
      setLog("魔王が　あらわれた！\n\n勝てる気は　しない。",true);
    }
  }
  ...
  function beginDialogue(){
    ...
    setDecisionLabel("次へ");
    dialogueIndex=0;
    currentSession=Math.min(talkCount,dialogueSessions.length-1);
    selected=4;

    var session=dialogueSessions[currentSession];
    setLog("SESSION "+String(talkCount+4).padStart(2,"0")+"\n\n"+session[dialogueIndex],false);
    note(523,.11,.015,"triangle");
  }
  ...
  function advanceDialogue(){
    ...
    if(dialogueIndex<session.length){
      setLog(session[dialogueIndex],false);

      if(dialogueIndex%2===1){
        note(220,.045,.006,"triangle");
      }else{
        note(330,.045,.006,"square");
      }
      return;
    }
    ...
    talkCount+=1;
    try{
      localStorage.setItem("storyBug017TalkCount",String(talkCount));
    }catch(e){}

    if(talkCount>=dialogueSessions.length){
      startSaveDeletion();
      return;
    }
  ...
  function finishDialogueSession(){
    phase="complete";
    stopBgm();
    ...
    copy.innerHTML=
      'SESSION '+String(talkCount+3).padStart(2,"0")+' SAVED.<br><br>'+
      'NEXT SESSION<br>'+
      String(talkCount+4).padStart(2,"0")+'<br><br>'+
      'THE BATTLE<br>'+
      'HAS NOT ENDED.<br><br>'+
      '<span class="sb-system-update">IT HAS NOT STARTED.</span>';

    overlay.classList.add("show");
    setDecisionLabel("終了");

    try{
      localStorage.setItem("storyBug017Cleared","1");
    }catch(e){}
  }

  function startSaveDeletion(){
    phase="deleting";
    stopBgm();
    playCurseBgm();
    ...
    copy.innerHTML='<span class="sb-save-delete">CORRUPTED</span>';
    overlay.classList.add("show");
    setDecisionLabel("…");
    ...
    function next(){
      ...
      if(i<steps.length){
        ...
        copy.innerHTML='<span class="sb-save-delete">'+steps[i][1]+'</span>';
        i+=1;
        resetSequenceTimer=setTimeout(next,1100);
        return;
      }
  ...
  function playCurseBgm(){
    var ctx=ensureAudio();
    if(!ctx)return;

    var base=ctx.currentTime+.03;
    var notes=[220,207.65,185,164.8,146.8,130.8,116.5,92.5,73.4,55];

    notes.forEach(function(freq,index){
      var t=base+index*.24;
      note(freq,.42,.025,index%2?"sawtooth":"square",t);
      note(freq*.5,.55,.013,"sine",t+.03);
    });

    for(var i=0;i<18;i++){
      var t2=base+i*.13;
      note(44+(i%3)*7,.12,.008,"square",t2);
    }
  }

  function performStoryReset(){
    resetCount+=1;

    try{
      localStorage.removeItem("storyBug017Defeats");
      localStorage.removeItem("storyBug017TalkCount");
      localStorage.removeItem("storyBug017Cleared");
      localStorage.setItem("storyBug017ResetCount",String(resetCount));
    }catch(e){}
    ...
    copy.innerHTML=
      'RESET COUNT: '+resetCount+'<br><br>'+
      '<span class="sb-system-update">RETURNING TO CARD...</span>';

    resetSequenceTimer=setTimeout(function(){
      close();
    },1800);
  }
  ...
  function keyDown(e){
    ...
    if(phase==="gameover"||phase==="dialogue"||phase==="complete"){
  ...
  function setDecisionLabel(label){
    var btn=root&&root.querySelector(".sb-btn.ok");
