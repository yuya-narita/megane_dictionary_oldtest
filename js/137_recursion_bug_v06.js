...
(function(){
  ...
  function ensureStyle(){
    if(document.getElementById("recursionBugStyleV06"))return;
    var st=document.createElement("style");st.id="recursionBugStyleV06";
    st.textContent=`
      ...
      .rb-stick-zone{position:absolute;display:grid;place-items:center;color:rgba(238,246,255,.72);font:1000 25px/1 ui-monospace,monospace}.rb-stick-left{left:0;top:0;bottom:0;width:36%}.rb-stick-right{right:0;top:0;bottom:0;width:36%}.rb-stick-down{left:40%;right:40%;top:55%;bottom:0}.rb-stick-center{position:absolute;left:50%;top:35%;width:42px;height:42px;transform:translate(-50%,-50%);border:2px solid rgba(176,190,255,.35);border-radius:50%;background:rgba(120,141,255,.08);pointer-events:none;transition:transform .08s ease,background .08s ease}.rb-stick[data-dir="left"] .rb-stick-center{transform:translate(calc(-50% - 34px),-50%);background:rgba(120,141,255,.28)}.rb-stick[data-dir="right"] .rb-stick-center{transform:translate(calc(-50% + 34px),-50%);background:rgba(120,141,255,.28)}.rb-stick[data-dir="down"] .rb-stick-center{transform:translate(-50%,calc(-50% + 34px));background:rgba(120,141,255,.28)}
  ...
  function newState(){return {mode:"title",grid:emptyBoard(),piece:null,nextType:null,bag:[],score:0,lines:0,level:1,last:0,fallClock:0,fallMs:720,grounded:false,lockClock:0,lockResets:0,hardDropped:false,swapUsed:false}}
  ...
  function spawn(){var type=state.nextType||nextFromBag();state.nextType=nextFromBag();state.piece={type:type,cells:cloneCells(SHAPES[type]),x:0,y:-1};normalize(state.piece);state.piece.x=Math.floor((COLS-pieceWidth(state.piece))/2)-minX(state.piece);state.grounded=false;state.lockClock=0;state.lockResets=0;state.hardDropped=false;state.swapUsed=false;if(collides(state.piece,0,0))gameOver();drawNext()}
  ...
  window.MEGANE_RECURSION_BUG={version:"v0.6-stable-swap-double-a",open:open,close:close};
})();
