/* 123_peace_shooter_v03.js
   SYNTAX FRONTIER INC.
   BUG No.012 — PEACE SHOOTER v0.3
...
(function(){
  ...
  function makeState(){
    return {
      ...
      boss:false,bossHP:9999,bossClock:0,bossSurvive:0,
  ...
  function renderEntities(){
    ...
    if(state.boss){
      ...
      boss.className="ps-entity ps-boss";boss.style.left="50%";boss.style.top="22%";
      boss.innerHTML='<span class="ps-boss-core"></span>';s.appendChild(boss);
      var hp=document.createElement("div");
      hp.className="ps-boss-hp";hp.textContent="BOSS HP "+state.bossHP;s.appendChild(hp);
    }
    ...
    if(state.boss){
      var alert2=document.createElement("div");alert2.className="ps-alert";alert2.textContent="BOSS APPROACH";s.appendChild(alert2)
    }
  }
  ...
  function spawnBoss(){
    state.boss=true;state.hostile=true;state.enemyBullets=[];state.playerBullets=[];
    var mode=root.querySelector(".ps-state");if(mode)mode.textContent="WAR∞";
  ...
  function loop(now){
    ...
    if(state.boss){
      state.bossClock+=dt;state.bossSurvive+=dt;
    ...
    if(!state.boss&&state.playerX>=88&&state.playerY<=30){
      clearGame();
      return;
    }
  ...
  window.MEGANE_PEACE_SHOOTER={version:"v0.3",open:open,close:close};
})();