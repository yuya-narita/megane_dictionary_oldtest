...
(function(){
  ...
  var root=null,state=null,audio=null,raf=0,timers=[],dbPromise=null,saveTimer=0,saveChain=Promise.resolve(),booting=false,mediaGesture=null;
  ...
  function ensureStyle(){
    if(document.getElementById("approvalBugStyleV04D"))return;
    var s=document.createElement("style");
    s.id="approvalBugStyleV04D";
    s.textContent=`
  ...
  function renderFeed(){
    ...
    feed.innerHTML=state.posts.slice().reverse().map(function(p){
      ...
      return '<article class="ab-post" data-id="'+p.id+'">'+
        ...
        '<div class="ab-post-stats"><span class="ab-stat">◌ '+fmt(p.replies||0)+'</span><span class="ab-stat">↻ '+fmt(p.reposts||0)+'</span><span class="ab-stat like '+(p.selfLiked?"self-liked":"")+'"><span class="ab-heart">'+(p.selfLiked?"♥":"♡")+'</span> <i>'+fmt(p.likes||0)+'</i></span><span class="ab-stat">⌑ '+fmt(p.saves||0)+'</span></div>'+
