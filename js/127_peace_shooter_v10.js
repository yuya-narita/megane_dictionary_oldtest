/* 127_peace_shooter_v10.js
   SYNTAX FRONTIER INC.
   BUG No.019 — PEACE SHOOTER v1.0
...
(function(){
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .ps-peace-bird{position:absolute;width:25px;height:12px;opacity:.50;z-index:7;pointer-events:none}
      .ps-peace-bird::before,.ps-peace-bird::after{content:"";position:absolute;top:4px;width:14px;height:8px;border-top:3px solid #17341f;border-radius:50%}
      .ps-peace-bird::before{right:50%;transform:rotate(10deg);transform-origin:right center}
      .ps-peace-bird::after{left:50%;transform:rotate(-10deg);transform-origin:left center}
      .ps-peace-bird.bird-near{transform:scale(1.08);opacity:.58}
      .ps-war-shade{position:absolute;inset:0;z-index:6;pointer-events:none;background:linear-gradient(180deg,rgba(7,18,12,.54) 0%,rgba(12,25,16,.34) 48%,rgba(18,36,23,.18) 100%);opacity:0;transition:opacity 1.7s ease}
      .ps-war-shade.active{opacity:.44}
      .ps-war-shade.boss{opacity:.62}
  ...
  function makeState(){
    return {
      ...
      birds:[{x:-12,y:23,speed:5.0,phase:0},{x:-45,y:39,speed:3.6,phase:1.7}],
      enemyClock:0,
    ...
    root.innerHTML=`
      <div class="ps-shell">
        <header class="ps-head">
          <span>SYNTAX FRONTIER INC. / BUG No.019</span>
          ...
          <div class="ps-stage">
            <div class="ps-title-screen">
              <div>
                ...
                <div class="ps-sub">BELIEF BUG / 199X</div>
  ...
  function render(){
    ...
    s.querySelectorAll(".ps-entity,.ps-bullet,.ps-alert,.ps-boss-hp,.ps-scenery,.ps-peace-bird,.ps-status-ticker,.ps-warning-line").forEach(function(el){el.remove()});
    ...
    var shade=s.querySelector(".ps-war-shade");if(shade){shade.classList.toggle("active",state.hostile||state.boss);shade.classList.toggle("boss",state.boss)}
    if(!state.hostile&&!state.boss){state.birds.forEach(function(birdState,index){var bird=document.createElement("div");bird.className="ps-peace-bird "+(index===0?"bird-near":"");bird.style.left=birdState.x+"%";bird.style.top=(birdState.y+Math.sin(state.sceneClock*2.2+birdState.phase)*1.6)+"%";s.appendChild(bird)})}else{for(var si=0;si<3;si++){var smoke=document.createElement("div");smoke.className="ps-scenery ps-war-smoke";smoke.style.left=(15+si*34)+"%";smoke.style.top=(12+((state.sceneClock*10+si*27)%74))+"%";s.appendChild(smoke)}var warning=document.createElement("div");warning.className="ps-warning-line";warning.style.top=(20+((state.sceneClock*18)%60))+"%";s.appendChild(warning)}
  ...
  function loop(now){
    ...
    state.worldDistance+=scrollSpeed*dt;state.sceneClock+=dt;state.tickerClock+=dt;if(!state.hostile&&!state.boss){state.birds.forEach(function(birdState,index){birdState.x+=birdState.speed*dt;if(birdState.x>112){birdState.x=-18-index*22;birdState.y=18+Math.random()*28}})}if(state.tickerClock>=2.2){state.tickerClock=0;if(state.boss)state.tickerText="CONFLICT STATUS: ONGOING";else if(state.hostile)state.tickerText="DEFENSE NETWORK: ACTIVE";else state.tickerText=["ROUTE STATUS: QUIET","NO THREAT DETECTED","CIVIL TRAFFIC: NORMAL"][Math.floor(state.sceneClock/2.2)%3]}
  ...
  function unresolvedEnding(){
    ...
    bindReturn("meganeBugFragment019B");
  }
  ...
  function clearGame(){
    ...
    bindReturn("meganeBugFragment019A");
  }
