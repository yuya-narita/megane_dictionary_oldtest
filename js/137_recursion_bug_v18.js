/* 137_recursion_bug_v18.js
   SYNTAX FRONTIER INC.
   BUG No.023 — RECURSION BUG
   ENDLESS STACK v1.8 / ENDING DIRECTOR'S CUT
...
(function(){
  ...
  function ensureStyle(){
    if(document.getElementById("recursionBugStyleV18"))return;
    var st=document.createElement("style");st.id="recursionBugStyleV18";
    st.textContent=`
      ...
      .rb-dev{position:absolute;inset:0;z-index:140;display:none;overflow-x:hidden;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;padding:max(18px,env(safe-area-inset-top)) 14px max(64px,calc(env(safe-area-inset-bottom) + 40px));background:rgba(3,5,11,.97);color:#eef6ff;touch-action:pan-y}
      #recursionBugRoot .rb-dev,#recursionBugRoot .rb-dev *{touch-action:pan-y !important}
      #recursionBugRoot .rb-dev button{touch-action:manipulation !important}
  ...
  function installZoomGuard(){
    ...
    function inDeveloper(e){return !!(e&&e.target&&e.target.closest&&e.target.closest(".rb-dev"))}
    function stopNativeZoom(e){if(inside(e)){e.preventDefault()}}
    function stopRootTouch(e){if(inside(e)&&!inDeveloper(e)){e.preventDefault()}}
  ...
  function open(){
    ...
    root.innerHTML=`<div class="rb-shell">
      ...
      <main class="rb-main"><div class="rb-stage"><canvas class="rb-canvas"></canvas><div class="rb-scan"></div>
        ...
        <div class="rb-dev">
          ...
          <div class="rb-dev-note">OPEN: PAUSE → DEV TEST / header long press / keyboard F2</div>
        </div>
        ...
        <div class="rb-pause"><div><div class="rb-over-title">PAUSED</div><button type="button" class="rb-menu-btn rb-resume">RESUME</button><button type="button" class="rb-menu-btn rb-dev-menu">DEV TEST</button><button type="button" class="rb-menu-btn rb-restart">RESTART</button><button type="button" class="rb-menu-btn danger rb-quit">QUIT</button></div></div>
      </div></main>
    ...
    $(".rb-resume").addEventListener("pointerup",function(e){block(e);resume()},true);$(".rb-dev-menu").addEventListener("pointerup",function(e){block(e);openDeveloper()},true);$(".rb-restart").addEventListener("pointerup",function(e){block(e);start()},true);$(".rb-quit").addEventListener("pointerup",stopEventClose,true);$(".rb-ending-return").addEventListener("pointerup",function(e){block(e);start()},true);
  ...
  function bindDeveloperPanel(){
    var titleText=root.querySelector(".rb-head > span");
    var panel=$(".rb-dev");
    ...
    titleText.style.webkitUserSelect="none";
    titleText.addEventListener("pointerdown",beginPress,true);
    titleText.addEventListener("pointerup",finishPress,true);
    titleText.addEventListener("pointercancel",finishPress,true);
    titleText.addEventListener("touchstart",beginPress,{capture:true,passive:false});
    titleText.addEventListener("touchend",finishPress,{capture:true,passive:false});
    titleText.addEventListener("touchcancel",finishPress,{capture:true,passive:false});

    $(".rb-dev-close").addEventListener("pointerdown",function(e){block(e);closeDeveloper()},true);
    $(".rb-dev-close").addEventListener("click",function(e){block(e);closeDeveloper()},true);

    panel.addEventListener("pointerup",function(e){
      var b=e.target.closest(".rb-dev-btn");
      if(!b)return;
      block(e);
  ...
  function debugAction(action){
    ...
    setTimeout(function(){
      ...
      else if(action==="ending-text"){if(!(COLS===40&&ROWS===80))debugFieldJump("40x80");startEnding();setTimeout(function(){var c=$(".rb-cosmos");if(c)c.classList.add("phase-title")},120)}
    },40);
  }
  ...
  window.MEGANE_RECURSION_BUG={version:"v1.8-ending-directors-cut",open:open,close:close};
})();
