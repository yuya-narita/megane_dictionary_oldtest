/* 139_approval_bug_final_clean_stats.js
...
(function(){
  ...
  var VERSION="final-1.0-clean-stats";
  ...
  function ensureStyle(){
    if(document.getElementById("approvalBugStyleFinalCleanStats"))return;
    var s=document.createElement("style");
    s.id="approvalBugStyleFinalCleanStats";
    s.textContent=`
      ...
      #approvalBugRoot .ab-mark{
        width:48px;height:34px;border-radius:11px;display:flex;align-items:center;justify-content:center;
        background:linear-gradient(145deg,var(--accent),var(--accent2));color:#fff;font-weight:1000;
        box-shadow:0 5px 18px rgba(99,91,255,.22);overflow:hidden
      }
      #approvalBugRoot .ab-echo-logo{gap:0;padding-left:4px}
      #approvalBugRoot .ab-echo-e{font-size:20px;line-height:1;font-weight:1000;letter-spacing:-.08em}
      #approvalBugRoot .ab-wave{display:inline-block;font-size:18px;line-height:1;margin-left:-3px;opacity:.86;transform-origin:left center}
      #approvalBugRoot .ab-wordmark{font-size:15px;letter-spacing:.18em}
      #approvalBugRoot .ab-brand.echoing .ab-wave-1{animation:abWavePulse .42s ease 1}
      #approvalBugRoot .ab-brand.echoing .ab-wave-2{animation:abWavePulse .42s .07s ease 1}
      #approvalBugRoot .ab-brand.echoing .ab-wave-3{animation:abWavePulse .42s .14s ease 1}
      #approvalBugRoot .ab-splash{
        position:absolute;inset:0;z-index:300;display:none;place-items:center;background:#0b0e17;color:#fff;
        transition:opacity .35s ease;pointer-events:none
      }
      #approvalBugRoot .ab-splash.show{display:grid}
      #approvalBugRoot .ab-splash.hide{opacity:0}
      #approvalBugRoot .ab-splash-logo{display:flex;align-items:center;justify-content:center;font-weight:1000}
      #approvalBugRoot .ab-splash-e{font-size:72px;line-height:1;letter-spacing:-.08em}
      #approvalBugRoot .ab-splash-wave{font-size:60px;line-height:1;margin-left:-13px;opacity:.08}
      #approvalBugRoot .ab-splash.show .w1{animation:abSplashWave .55s .12s ease forwards}
      #approvalBugRoot .ab-splash.show .w2{animation:abSplashWave .55s .24s ease forwards}
      #approvalBugRoot .ab-splash.show .w3{animation:abSplashWave .55s .36s ease forwards}
      #approvalBugRoot .ab-splash-word{margin-top:-28vh;font-size:12px;font-weight:850;letter-spacing:.48em;color:#b8c1d6}
      @keyframes abWavePulse{0%{opacity:.15;transform:scale(.72)}55%{opacity:1;transform:scale(1.22)}100%{opacity:.86;transform:scale(1)}}
      @keyframes abSplashWave{0%{opacity:.08;transform:translateX(-7px) scale(.7)}65%{opacity:1;transform:translateX(0) scale(1.12)}100%{opacity:.8;transform:scale(1)}}
      ...
      #approvalBugRoot .ab-viral.show{display:block;animation:abViralFlash .48s steps(2,end) infinite;box-shadow:inset 0 0 120px rgba(99,91,255,.18)}
  ...
  function build(){
    ...
    root.innerHTML=`
      <div class="ab-shell">
        <div class="ab-splash show" aria-hidden="true">
          <div class="ab-splash-logo">
            <span class="ab-splash-e">E</span>
            <span class="ab-splash-wave w1">)</span>
            <span class="ab-splash-wave w2">)</span>
            <span class="ab-splash-wave w3">)</span>
          </div>
          <div class="ab-splash-word">E C H O</div>
        </div>
        <header class="ab-top">
          <div class="ab-brand" aria-label="ECHO">
      <span class="ab-mark ab-echo-logo">
        <span class="ab-echo-e">E</span>
        <span class="ab-wave ab-wave-1">)</span>
        <span class="ab-wave ab-wave-2">)</span>
        <span class="ab-wave ab-wave-3">)</span>
      </span>
      <span class="ab-wordmark">ECHO</span>
    </div>
        ...
        <div class="ab-dev-panel">
          <div class="ab-dev-card">
            ...
            <div class="ab-dev-note">E)))ロゴを約1.2秒長押しで開閉。完全リセットは投稿・画像・プロフィール・通知・バズ状態を削除します。</div>
          </div>
        </div>
    ...
    autoGrowInput();
    later(function(){
      var splash=$(".ab-splash");
      if(!splash)return;
      splash.classList.add("hide");
      later(function(){if(splash)splash.classList.remove("show")},360);
    },900);
  ...
  function renderProfile(){
    ...
    $(".ab-profile").innerHTML=
      ...
      '</div></div>'+
      note+
      '<div class="ab-system-note" style="background:transparent;color:var(--muted);border:1px solid var(--line)">'
      +'<div style="display:grid;grid-template-columns:1fr auto;gap:9px 14px;align-items:center">'
      +'<span>最高反応</span><b style="color:var(--text)">'+(topPost?fmt(topPost.likes)+" いいね":"—")+'</b>'
      +'<span>累計反応</span><b style="color:var(--text)">'+fmt(state.totalLikes)+' いいね</b>'
      +'<span>プロフィール閲覧</span><b style="color:var(--text)">'+fmt(state.profileViews)+'</b>'
      +'</div></div>'+
  ...
  function pulseEchoLogo(){
    var brand=$(".ab-brand");
    if(!brand)return;
    brand.classList.remove("echoing");
    void brand.offsetWidth;
    brand.classList.add("echoing");
    later(function(){if(brand)brand.classList.remove("echoing")},700);
  }
  ...
  function burstNotification(post,t){
    ...
    $(".ab-bell").classList.remove("ring");void $(".ab-bell").offsetWidth;$(".ab-bell").classList.add("ring");pulseEchoLogo();
  }
  ...
  function addReactionNotification(post,loud){
    ...
    if(loud)ping(.018);pulseEchoLogo();
  }
