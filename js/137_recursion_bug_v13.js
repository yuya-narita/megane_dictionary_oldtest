/* 137_recursion_bug_v13.js
   SYNTAX FRONTIER INC.
   BUG No.023 — RECURSION BUG
   ENDLESS STACK v1.3 / GUARANTEED DEVELOPER ENTRY
...
(function(){
  ...
  function ensureStyle(){
    if(document.getElementById("recursionBugStyleV13"))return;
    var st=document.createElement("style");st.id="recursionBugStyleV13";
    st.textContent=`
  ...
  function open(){
    ...
    root.innerHTML=`<div class="rb-shell">
      <header class="rb-head"><span>SYNTAX FRONTIER INC. / BUG No.023</span><div class="rb-head-actions"><button type="button" class="rb-icon rb-dev-open" aria-label="Developer">DEV</button><button type="button" class="rb-icon rb-pause-btn" aria-label="一時停止">Ⅱ</button><button type="button" class="rb-icon rb-close" aria-label="閉じる">×</button></div></header>
      <main class="rb-main"><div class="rb-stage"><canvas class="rb-canvas"></canvas><div class="rb-scan"></div>
        ...
        <div class="rb-dev">
          ...
          <div class="rb-dev-note">OPEN: PAUSE → DEV TEST / top DEV / keyboard F2</div>
        </div>
    ...
    installZoomGuard();
    $(".rb-close").addEventListener("pointerup",stopEventClose,true);$(".rb-pause-btn").addEventListener("pointerup",function(e){block(e);togglePause()},true);$(".rb-dev-open").addEventListener("pointerup",function(e){block(e);openDeveloper()},true);
  ...
  window.MEGANE_RECURSION_BUG={version:"v1.3-guaranteed-developer-entry",open:open,close:close};
})();
