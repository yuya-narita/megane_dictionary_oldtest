...
(function(){
  ...
  var VERSION="v0.2-profile-media-world";
  var STORE_KEY="looking_bear_approval_bug_v02";
  ...
  function ensureStyle(){
    if(document.getElementById("approvalBugStyleV02"))return;
    var s=document.createElement("style");
    s.id="approvalBugStyleV02";
    s.textContent=`
      ...
      #approvalBugRoot .ab-media-thumb{margin:10px 0 0 54px;border-radius:15px;overflow:hidden;border:1px solid var(--line);background:#eef1f6}
      #approvalBugRoot .ab-media-thumb img{display:block;width:100%;max-height:360px;object-fit:cover}
      ...
      #approvalBugRoot .ab-field input,#approvalBugRoot .ab-field textarea{width:100%;border:1px solid var(--line);border-radius:12px;background:var(--bg);color:var(--text);padding:11px;font:inherit}
      #approvalBugRoot .ab-field textarea{min-height:78px}
        ...
        <main class="ab-main">
          <section class="ab-view ab-home show">
            <div class="ab-compose">
              ...
              <input class="ab-post-image-input" type="file" accept="image/*" hidden>
              <div class="ab-compose-foot"><div class="ab-tools"><button type="button" class="ab-small-btn ab-add-image">画像</button><span class="ab-image-chip"></span></div><button class="ab-post-btn" type="button">投稿</button></div>
            </div>
  ...
  function bind(){
    ...
    $(".ab-add-image").addEventListener("pointerdown",function(e){block(e);$(".ab-post-image-input").click()},true);
  ...
  function saveProfile(){
    ...
    save();closeProfileModal();renderAll();toast("プロフィールを保存しました");
  }
  ...
  function renderProfile(){
    ...
    $(".ab-profile").innerHTML=
      ...
      note+
      '<div class="ab-system-note" style="background:transparent;color:var(--muted);border:1px solid var(--line)">最高反応　'+(topPost?fmt(topPost.likes)+" いいね":"—")+'<br>累計反応　'+fmt(state.totalLikes)+' いいね<br>プロフィール閲覧　'+fmt(state.profileViews)+'</div>'+
  ...
  function createPost(text){
    state.postCount++;
    var result=engagementFor(state.postCount,state.phase,!!state._pendingPostImage);
    var post={
  ...
  function submitPost(){
    ...
    if(!text)text="";
    input.value="";var prev=$(".ab-image-preview");prev.src="";prev.classList.remove("show");$(".ab-image-chip").textContent="";updatePostButton();
